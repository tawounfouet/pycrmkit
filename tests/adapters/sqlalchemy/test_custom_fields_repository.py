"""Run Custom Field repository contracts against the SQLAlchemy adapter."""
...
from pycrmkit.storage.sqlalchemy.repositories import SQLAlchemyCustomFieldRepository
...
def test_sqlalchemy_custom_field_repository_passes_contract(session: Session) -> None:
    assert_custom_field_repository_contract(SQLAlchemyCustomFieldRepository(session))


def test_custom_field_repository_isolates_definition_mutations(
    session: Session,
) -> None:
